...
class BatchGenerator(object):
    def __init__(self, num_classes_gestures, num_classes_tools, actions_dict_gestures, actions_dict_tools,
                 features_path, split_num, folds_folder, gt_path_gestures=None, gt_path_tools_left=None,
                 gt_path_tools_right=None, sample_rate=1, normalization="None", task="gestures"):
        """
    ...
    def normalization_params_read(self):
        params = pd.read_csv(os.path.join(self.folds_folder, "std_params_fold_" + str(self.split_num) + ".csv"),
                             index_col=0).values
        self.max = params[0, :]
        self.min = params[1, :]
        self.mean = params[2, :]
    ...
    def next_batch(self, batch_size):
        ...
        if self.task == "gestures":
            ...
            for i in range(len(batch_input)):
                ...
                batch_target_tensor[i, :np.shape(batch_target_gestures[i])[0]] = torch.from_numpy(
                    batch_target_gestures[i])
                mask[i, :, :np.shape(batch_target_gestures[i])[0]] = torch.ones(self.num_classes_gestures,
            ...
            length_of_sequences = list(np.min(
                np.concatenate((length_of_sequences_left, length_of_sequences_right, length_of_sequences_gestures), 1),
                1))
